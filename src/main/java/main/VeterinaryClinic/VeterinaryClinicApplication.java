...
//	 }
	...
	public CommandLineRunner mappingDemo(MedicineService medicineService,
		...
		return args -> {
			...
			wareHouseService.create(new WareHouse(data, 100,"medicine",6500,"2022/01/22"));
			wareHouseService.create(new WareHouse(data, 200,"medicine",13000,"2022/01/22"));

			data = new Medicine("Tramadol HCL","capsule",15,"ครั้งละ 1 เม็ด หลังอาหารเช้า เย็น","50 mg");
			medicineService.create(data);
			wareHouseService.create(new WareHouse(data, 120,"medicine",1440,"2022/01/27"));

			data = new Medicine("Toflex/Cephalexin","capsule",20,"ครั้งละ 1 เม็ด หลังอาหารเช้า เย็น","250mg");
			medicineService.create(data);
			wareHouseService.create(new WareHouse(data, 200,"medicine",3600,"2022/02/12"));


			data = new Medicine("Hepato Support","capsule",35,"ครั้งละ 1 เม็ด หลังอาหารย็น");
			medicineService.create(data);
			wareHouseService.create(new WareHouse(data, 100,"medicine",3300,"2022/01/06"));

			//Create tool connect to warehouse
			Tool toolData = new Tool("Syringe",10,"5 ml");
			toolService.create(toolData);
			wareHouseService.create(new WareHouse(toolData, 100,"tool",800,"2022/01/12"));
			wareHouseService.create(new WareHouse(toolData, 300,"tool",24000,"2022/12/30"));

			toolData = new Tool("Syringe",5,"3 ml");
			toolService.create(toolData);
			wareHouseService.create(new WareHouse(toolData, 150,"tool",6000,"2022/01/12"));
